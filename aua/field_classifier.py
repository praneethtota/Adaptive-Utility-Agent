    ...
    def get_effective_config(self, distribution: dict[str, float]) -> FieldConfig:
        ...
        if entropy_ratio > ENTROPY_CONSERVATIVE_THRESHOLD:
            ...
            most_conservative = max(
                distribution, key=lambda f: FIELD_CONFIGS.get(f, FIELD_CONFIGS["general"]).c_min
            )
            ...
            base_config.c_min = base_config.c_min + lerp_t * (
                conservative_cfg.c_min - base_config.c_min
            )
            base_config.e_min = base_config.e_min + lerp_t * (
                conservative_cfg.e_min - base_config.e_min
            )
