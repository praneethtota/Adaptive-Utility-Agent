...
class UtilityScorer:
    ...
    def score(
        ...
        active_gap_subject: str | None = None,  # subject with active gap bonus
    ) -> TaskScore:
    ...
    def get_domain_summary(self, domain: str) -> dict:
        ...
        return {
            ...
            "contradiction_rate": round(
                state.contradiction_count / max(state.interaction_count, 1), 4
            ),
            "recommended_difficulty": self._recommended_difficulty(state.confidence),
        }

    def get_utility_trend(self, domain: str | None = None, last_n: int = 20) -> list[float]:
    ...
    def _update_efficacy_ema(self, state: DomainState, efficacy_raw: float) -> float:
        ...
        state.efficacy_ema = (
            1 - self.EFFICACY_ALPHA
        ) * state.efficacy_ema + self.EFFICACY_ALPHA * efficacy_raw
    ...
    def _update_confidence(
        ...
        state.confidence = (
            1 - self.CONFIDENCE_ALPHA
        ) * state.confidence + self.CONFIDENCE_ALPHA * penalized_signal
