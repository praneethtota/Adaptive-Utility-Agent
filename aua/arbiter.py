...
class ArbiterVerdict:
    ...
    verified_claim: str | None = None  # what the correct answer is
...
class ArbiterAgent:
    ...
    def arbitrate(
        ...
        claimed_complexity_A: str | None = None,
        claimed_complexity_B: str | None = None,
    ) -> ArbiterVerdict:
        """
    ...
    def get_gap_bonus(
        ...
        total_gap_demand = sum(
            k_effective * g.gap_multiplier for g in self.active_gaps.values() if not g.resolved
        )
    ...
    def _check_mathematical(
        ...
        output_A: str,
        output_B: str,
        complexity_A: str | None,
        complexity_B: str | None,
    ) -> CheckResult:
        """
        ...
        if a_wrong and not b_wrong:
            return CheckResult(
                ...
                True,
                "B",
                f"A claims {complexity_A} but code structure contradicts it",
                0.85,
            )
        if b_wrong and not a_wrong:
            return CheckResult(
                ...
                True,
                "A",
                f"B claims {complexity_B} but code structure contradicts it",
                0.85,
            )
        if a_wrong and b_wrong:
            return CheckResult(
                "mathematical", True, "neither", "Both have complexity claim mismatches", 0.75
            )
    ...
    def _check_cross_session(
        ...
        if not matches:
            return CheckResult(
                "cross_session", False, None, "No prior assertions on this subject", 0.0
            )
        ...
        if a_contradicts and b_contradicts:
            return CheckResult(
                ...
                True,
                "neither",
                "Both contradict stored assertion. " + explanation,
                eff_conf * 0.7,
            )
    ...
    def _compute_confidence(self, checks: list[CheckResult]) -> float:
        ...
        """
        numerator = sum(CHECK_WEIGHTS[c.check_type] * (1.0 if c.converged else 0.0) for c in checks)
    ...
    def _build_evidence_summary(self, checks: list[CheckResult], winner: str) -> str:
        ...
        for c in checks:
            if c.converged:
                parts.append(
                    f"  [{c.check_type}] winner={c.winner} conf={c.confidence:.2f}: {c.explanation}"
                )
        return " | ".join(parts)
