...
class QueryRequest(BaseModel):
    ...
    force_domain: str | None = None  # override routing for testing
...
class RouterResponse(BaseModel):
    ...
    latency_ms: float
    specialist_responses: list[dict] | None = None
...
class Router:
    ...
    async def _handle_single(
        ...
        if spec_obj:
            self._requests_per_spec[spec_obj.name] = (
                self._requests_per_spec.get(spec_obj.name, 0) + 1
            )
