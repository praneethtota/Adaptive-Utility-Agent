...
class SpecialistConfig:
    """One vLLM specialist server."""
    name: str                          # e.g. "swe"
    model: str                         # HuggingFace repo or local path
    port: int                          # vLLM API port
    field: str                         # maps to a key in FIELD_CONFIGS
    gpu: int = 0                       # CUDA device index
    gpu_memory_utilization: float = 0.34
    max_model_len: int = 2048
    quantization: Optional[str] = "awq"   # "awq" | "gptq" | None (fp16)
    enforce_eager: bool = True         # prevents CUDA graph profiling conflicts
...
class AUAConfig:
    ...
    def blue_green_for(self, specialist_name: str) -> BlueGreenFieldConfig:
        ...
        return self.blue_green.get(
            specialist_name, BlueGreenFieldConfig()
        )
