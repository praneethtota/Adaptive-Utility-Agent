...
DECAY_TAU_SECONDS: dict[DecayClass, float | None] = {
...
class Assertion:
    ...
    def is_trustworthy(self, threshold: float = 0.5, now: float | None = None) -> bool:
...
class AssertionsStore:
    ...
    def add(
        ...
        decay_class_override: DecayClass | None = None,
    ) -> Assertion:
    ...
    def prune_stale(self, min_effective_confidence: float = 0.05) -> int:
        ...
        self.assertions = [
            a for a in self.assertions if a.effective_confidence(now) >= min_effective_confidence
        ]
